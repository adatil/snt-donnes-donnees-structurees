...
\item Téléchargez le fichier \textit{Films ayant réalisé plus d'un million d'entrées} depuis \url{https://www.data.gouv.fr/fr/} (recherchez "Films entrées")
...
\textit{Indice :} Utilisez la méthode \texttt{sum()} sur la colonne des entrées.

\vspace{0.5cm}
\noindent \fbox{\begin{minipage}{0.95\textwidth}
\textbf{Astuce :} La fonction \texttt{describe()} donne un résumé statistique complet d'une colonne. Cela affiche le minimum, maximum, moyenne, médiane, etc.
\end{minipage}}
...
\textit{Indice :} Convertissez d'abord la colonne "sortie" en datetime, puis filtrez sur l'année :
\begin{lstlisting}
